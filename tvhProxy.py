...
config = {
    'tvhproxyIpAddress':   str(os.getenv('TVHPROXY_IP_ADDRESS',             '127.0.0.1')),
    'friendlyName':        str(os.getenv('TVHPROXY_FRIENDLY_NAME',          'HDHR Emulator')),
    'modelNumber':         str(os.getenv('TVHPROXY_MODEL_NUMBER',           'HDTC-2US')),
    'manufacturer':        str(os.getenv('TVHPROXY_MANUFACTURER',           'Silicondust')),
    'firmwareName':        str(os.getenv('TVHPROXY_FIRMWARE_NAME',          'hdhomeruntc_atsc')),
    'firmwareVersion':     str(os.getenv('TVHPROXY_FIRMWARE_VERSION',       '20150826')),
    ...
    'chunkSize':           int(os.getenv('TVHPROXY_CHUNK_SIZE',             1024 * 1024)),
    'sortChannelsByField': str(os.getenv('TVHPROXY_SORT_CHANNELS_BY_FIELD', 'NONE')),
    'sortChannelsOrder':   str(os.getenv('TVHPROXY_SORT_CHANNELS_ORDER',    'ASC'))
}
    ...
    for tvheadendChannel in _get_tvheadend_channels():
        if tvheadendChannel['enabled']:
            if config['directStream'] == 'YES':
                url = '%s/stream/channel/%s?profile=%s&weight=%s' % (config['tvheadendUrl'], tvheadendChannel['uuid'], config['streamProfile'], config['subscriptionWeight'])
            else:
                url = '%s/auto/v%s' % (config['tvhproxyUrl'], tvheadendChannel['number'])
...
def stream(channel):
    ...
    if not url:
        ...
        def generate():
            yield ''
            for chunk in tvheadendRequest.iter_content(chunk_size=config['chunkSize']):
                if not duration == 0 and not time.time() < duration:
                    tvheadendRequest.close()
                    break
                yield chunk

        return Response(generate(), content_type=tvheadendRequest.headers['content-type'], direct_passthrough=True)
...
def _get_tvheadend_channels():
    ...
    try:
        ...
        if config['sortChannelsByField'] != 'NONE':
            tvheadendChannels = sorted(tvheadendChannels, key = itemgetter(config['sortChannelsByField']), reverse=reverseOrder)
