...
config = {
    'tvhproxyIpAddress': os.getenv('TVHPROXY_IP_ADDRESS',           '127.0.0.1'),
    'deviceId':          os.getenv('TVHPROXY_DEVICE_ID',            '12345678'),           # a hexadecimal string
    'tunerCount':        os.getenv('TVHPROXY_TUNER_COUNT',          4),                    # number of tuners in tvheadend
    'ignoreIpAddresses': os.getenv('TVHPROXY_IGNORE_IP_ADDRESSES',  '127.0.0.1').split(',') # the tvheadend ip address(es), tvheadend crashes when it discovers the tvhproxy (TODO: Fix this)
}
...
def processPacket(packet, client, logPrefix = ''):
    ...
    if packetType == HDHOMERUN_TYPE_DISCOVER_REQ:
        ...
        responsePayload += struct.pack('>BBI', HDHOMERUN_TAG_DEVICE_ID, 0x04, int(config['deviceId'], 16)) #Device ID Filter (any)
        responsePayload += struct.pack('>BB{0}s'.format(len(config['tvhproxyUrl'])), HDHOMERUN_TAG_GETSET_NAME, len(config['tvhproxyUrl']), config['tvhproxyUrl']) #Device ID Filter (any)
        responsePayload += struct.pack('>BBB', HDHOMERUN_TAG_TUNER_COUNT, 0x01, config['tunerCount']) #Device ID Filter (any)
    ...
    if packetType == HDHOMERUN_TYPE_GETSET_REQ:
        ...
        while True:
            ...
            if length > 127:
                print logPrefix + 'Unable to determine tag length, the correct way to determine a length larger than 127 must still be implemented.'
                return False
...
def tcpServer():
    ...
    controlSocket.bind((config['tvhproxyIpAddress'], HDHOMERUN_CONTROL_TCP_PORT))
            ...
            try:
                ...
                if client[0] not in config['ignoreIpAddresses']:
                    ...
                    if responsePacket:
                        ...
                        print logPrefix + 'No known control request received, nothing to send to client'
                elif args.verbose > 1:
    ...
    while True:
        ...
        if client[0] not in config['ignoreIpAddresses']:
            ...
            if responsePacket:
                ...
                print logPrefix + 'No discovery request received, nothing to send to client'
        elif args.verbose > 1:
